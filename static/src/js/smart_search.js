...
`;
    ...
    _loadChatHistory() {
        ...
        const key = `bizvibe_chat_history_${this.state.userId}`;

        const stored = localStorage.getItem(key);

        if (stored) {
            this.state.chatHistory = JSON.parse(stored);
        }
    ...
    _saveSearchHistory(query, response) {

        const key = `bizvibe_chat_history_${this.state.userId}`;

        let history = this.state.chatHistory || [];
